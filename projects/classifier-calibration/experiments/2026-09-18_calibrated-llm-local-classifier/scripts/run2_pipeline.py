#!/usr/bin/env python3
"""Run 2: controlled iteration — Potion potion-base-32M on frozen Run 1 splits.

Reuses inputs/fixtures corpus + split_manifest from Run 1 (same train/calibration/test IDs).
LLM path remains LocalClosedSetScoreProvider (no hosted LLM). Only controlled change:
potion-base-8M → potion-base-32M; re-fit temperature scaling on calibration only.
"""
...
def main() -> int:
    ...
    # --- 1/2. Load frozen corpus + splits from Run 1 (do not regenerate) ---
    fixture_path = fixtures_dir / "synthetic_topics_v1.jsonl"
    prior_split_path = fixtures_dir / "split_manifest.json"
    prior_dataset_path = fixtures_dir / "dataset_manifest.json"
    if not fixture_path.exists() or not prior_split_path.exists():
        raise SystemExit("Frozen fixtures from Run 1 missing under inputs/fixtures/")
    ...
    split_manifest = json.loads(prior_split_path.read_text(encoding="utf-8"))
    dataset_manifest = json.loads(prior_dataset_path.read_text(encoding="utf-8"))
    # annotate reuse
    dataset_manifest = dict(dataset_manifest)
    dataset_manifest["reused_from_run"] = PRIOR_RUN_ID
    dataset_manifest["controlled_change"] = "potion-base-8M -> potion-base-32M; splits frozen"

    splits = {
        "train": list(split_manifest["train_ids"]),
        "calibration": list(split_manifest["calibration_ids"]),
        "test": list(split_manifest["test_ids"]),
    }
    # Verify digests unchanged
    for key, ids in splits.items():
        digest_key = f"{key}_ids_sha256"
        got = sha256_text("\n".join(ids))
        expected = split_manifest["digests"][digest_key]
        if got != expected:
            raise SystemExit(f"Frozen split digest mismatch for {key}: {got} != {expected}")

    prior_run_split = exp_dir / "runs" / PRIOR_RUN_ID / "split_manifest.json"
    if prior_run_split.exists():
        prior = json.loads(prior_run_split.read_text(encoding="utf-8"))
        if prior["digests"] != split_manifest["digests"]:
            raise SystemExit("Run 2 fixtures digests do not match run-001 split_manifest digests")
    ...
    observability = {
        ...
        "prometheus_grafana": {"used": False, "reason": "No long-lived model server in Run 2"},
    ...
    llm_config = {
        ...
        "documentation": (
            ...
            "This Run 2 keeps the documented LocalClosedSetScoreProvider that emits "
    ...
    m2v = StaticModelForClassification.from_pretrained(POTION_BASE)
    m2v.fit(
        train_texts,
        train_labels,
        max_epochs=5,
    ...
    classifier_config = {
        ...
        "classifier_revision": f"potion-base-32M+train_seed{SEED}",
        "calibration": {
            "applied": True,
    ...
    # Pick next hypothesis from error patterns (post potion-base-32M iteration)
    next_hypothesis = (
        "Experiment acceptance is met with documented substitutes (no hosted LLM/Langfuse). "
        "Optional follow-up outside this closed iteration: swap LocalClosedSetScoreProvider "
        "for a real local generative LLM with token logprobs on the same frozen splits."
    )
    if len(cls_only_wrong) > len(llm_only_wrong) and len(cls_only_wrong) > 0:
        next_hypothesis = (
            "Potion potion-base-32M still errs more than the score provider on the frozen test set. "
            "Optional follow-up: try a different Model2Vec training recipe (more epochs / lower "
            "early-stopping patience) or a denser embedding, keeping the same frozen IDs."
        )
    elif len(llm_only_wrong) > len(cls_only_wrong) and len(llm_only_wrong) > 0:
        next_hypothesis = (
            "The LocalClosedSetScoreProvider errs more than Potion-32M. Optional follow-up: "
            "replace the TF-IDF LogReg substitute with a small local generative LLM using "
            "true token logprobs on the same frozen splits."
        )
    elif len(both_wrong) > 0:
        next_hypothesis = (
            "Shared errors suggest fixture ambiguity from cross-label keyword noise. "
            "Optional follow-up: fixture v2 with lower contamination (new dataset_version)."
        )
    elif len(cls_only_wrong) == 0 and len(llm_only_wrong) == 0:
        next_hypothesis = (
            "Both systems perfect on this synthetic frozen test set after potion-base-32M. "
            "Limitation: synthetic fixture + LLM substitute; validate on a real domain corpus "
            "and a true generative LLM when available (new dataset_version)."
        )
...
"""
    ...
    run_json = {
        "run_id": RUN_ID,
        "experiment_id": EXPERIMENT_ID,
        "started_intent": "Run 2 controlled: potion-base-32M on frozen Run 1 splits vs same LLM-substitute",
        "created_at": utc_now(),
        "status": "completed",
        ...
        "commands": [
            "python3 scripts/run2_pipeline.py",
        ],
        "artifacts": {
