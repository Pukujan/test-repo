...
def http_post(payload, api_key, timeout=60, attempts=3):
...
def classify(model, sample, split, api_key):
    system, user = common.prompt_pair(sample)
    ...
    base = {"schema": "mdrg-teacher-pred.v1", "sample_id": sample["id"], "split": split,
            ...
            "invocation_path": BASE_URL, "prompt_version": PROMPT_VERSION, "trace_id": trace_id}
    try:
...
def main():
    ...
    print(f"{args.split}: {len(rows)} rows, {len(done)} done, {len(todo)} to call")
    ...
    def work(s):
        nonlocal produced
        rec = classify(args.model, s, args.split, api_key)
        with lock:
    ...
    # committed observability (no gold, no raw text): rebuild from the teacher file.
    allrecs = [json.loads(l) for l in tpath.read_text(encoding="utf-8").splitlines()]
    events, traces = [], []
    for r in allrecs:
        if r["status"] != "ok":
            continue
    ...
    full = [r for r in ok if r.get("score_coverage") == "full"]
    errs = len(allrecs) - len(ok)
    echo = sorted({r.get("provider_echo_model") for r in ok})
    match = sum(1 for r in ok if r["teacher_domain"] == r["gold_domain"])
    print(f"{args.split}: ok={len(ok)} errors={errs} echo={echo} "
          f"full_score={len(full)} logprobs_all={all(r.get('logprobs_available') for r in ok) if ok else None}")
    print(f"{args.split}: teacher-vs-source-membership agreement on completed = {match}/{len(ok)} "
          f"({round(100*match/len(ok),2) if ok else 0}%)")
